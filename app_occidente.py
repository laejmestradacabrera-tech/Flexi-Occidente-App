...
# --- EL CARTERO LIGERO (COMPARATIVO MENSUAL) ---
...
# ========================================================
# --- CONTROL DE ACCESOS (BIFURCACIÓN DE PESTAÑAS) ---
st.markdown("<div style='background-color:#f8f9fa; padding:15px; border-radius:8px; border:1px solid #ddd; margin-bottom:20px;'>", unsafe_allow_html=True)
col_perfil1, col_perfil2 = st.columns([1, 2])
with col_perfil1:
    perfil_usuario = st.radio("👤 Tipo de Perfil:", ["Encargada de Tienda", "Gerencia Comercial"], horizontal=True)

acceso_gerencia = False
if perfil_usuario == "Gerencia Comercial":
    with col_perfil2:
        pwd_admin = st.text_input("🔑 Clave de Gerencia:", type="password", placeholder="Ingresa clave para desbloquear Módulo Gerencial")
        if pwd_admin == "T5604b":
            acceso_gerencia = True
            st.success("✅ Acceso Directivo Desbloqueado")
        elif pwd_admin != "":
            st.error("❌ Clave incorrecta. Modo restringido a Encargada.")
st.markdown("</div>", unsafe_allow_html=True)

# Asignación dinámica de pestañas según perfil (Sin Monitor Estratégico)
if acceso_gerencia:
    tab1, tab_rating, tab2, tab3, tab4, tab_bitacora, tab5, tab6, tab7 = st.tabs([
        "📊 DESEMPEÑO COMERCIAL", "🏆 RATING COMERCIAL", "📈 COMPARATIVO MENSUAL",
        "👟 TOP 20 TIENDA", "🌍 TOP 20 ZONA", "📝 BITÁCORA", 
        "🧭 RUTA DEL CLIENTE", "🎓 CAPACITACIÓN", "🔄 NIVELACIÓN DE STOCK"
    ])
else:
    # Perfil Encargada (Ocultamos Comparativo Mensual y Top 20 Zona)
    tab1, tab_rating, tab3, tab_bitacora, tab5, tab6, tab7 = st.tabs([
        "📊 DESEMPEÑO COMERCIAL", "🏆 RATING COMERCIAL", "👟 TOP 20 TIENDA", 
        "📝 BITÁCORA", "🧭 RUTA DEL CLIENTE", "🎓 CAPACITACIÓN", "🔄 NIVELACIÓN DE STOCK"
    ])
    tab2 = tab4 = None # Pestañas desactivadas para este perfil
...
# --- PESTAÑA: RATING COMERCIAL ---
with tab_rating:
    st.markdown("<h2 style='text-align: center; color: #EAB308;'>🏆 RATING COMERCIAL OCCIDENTE</h2>", unsafe_allow_html=True)
    st.info("Fase 1: Despliegue Visual del Tablero Gamificado. (Próximamente los puntos se conectarán en vivo con los cierres de Ventas y Bitácora).")
    ...
    try:
        if os.path.exists("Raiting Elegido..html"):
            ...
            st.warning("⚠️ El archivo 'Raiting Elegido..html' no se encontró en la carpeta. Súbelo a GitHub para visualizarlo.")
    except Exception as e:
        st.error(f"Error al cargar el Rating: {e}")

# --- PESTAÑA 2: COMPARATIVO MENSUAL (SOLO GERENCIA) ---
if tab2:
    with tab2:
        ...
        if archivo_comp:
            ...
            if c_prs and c_imp:
                ...
                # --- BOTÓN DE ENVÍO MANUAL (PROTEGIDO) ---
                st.write("<br>", unsafe_allow_html=True)
                st.markdown("---")
                if st.button("🚀 Enviar Reporte Ejecutivo del Día a Tiendas", type="primary"):
                    st.info("Función habilitada para envíos masivos. (Solo modo demo actual).")
    ...
    if st.button("💾 Guardar en Bitácora"):
        ...
        if "Faltante de Tallas" in factor:
            if not modelo_captura:
                ...
                st.error("❌ Por favor, ingresa un N° de Sucursal válido (SOLO NÚMEROS, ej. 56) en la parte superior para poder cruzar el inventario.")
                puede_guardar = False
            else:
                try:
        ...
        if puede_guardar:
            ...
            try:
                if 'sheet_bitacora' in globals():
                    ...
                    st.warning("⚠️ No se conectó a Google Sheets.")
            except Exception as e:
...
with tab6:
    ...
    with col_der:
        ...
        st.info("**🎯 Objetivo General:** Proceso de acogida estandarizado para reducir rotación en los primeros 90 días.")
        with st.expander("🎯 1. PROPÓSITO DEL MONITOR COMERCIAL"):
            st.markdown("Toda la capacitación se resume en dominar: **Ticket Promedio (1.29)** y **Conversión (10.90%)**.")

# --- PESTAÑA 8: NIVELACIÓN (TERRITORIO SAGRADO - INTACTO) ---
with tab7:
    ...
    if 'df_ventas' in st.session_state:
        ...
        if st.button("Ejecutar Análisis"):
            ...
            for _, row in df_top.iterrows():
                # Obtenemos las columnas de existencia y pedido
                cols_ex = [f'ex{i}' for i in range(1, 16) if f'ex{i}' in row.index]
                cols_p = [f'p{i}' for i in range(1, 16) if f'p{i}' in row.index]
                
                # Sumamos el total del modelo
                suma_ex = pd.to_numeric(row[cols_ex], errors='coerce').fillna(0).sum()
                suma_p = pd.to_numeric(row[cols_p], errors='coerce').fillna(0).sum()
                
                # Regla de Quiebre: 0 existencias Y 0 pedidos
                if suma_ex <= 0 and suma_p <= 0:
                    resultados.append({
                        "Departamento": str(row.get('Departamento', 'S/D')).capitalize(),
                        "Modelo": row['Modelo'],
                        "Pares Vendidos (Histórico)": row.get('Vtas', 0),
                        "Estatus": "🚨 QUIEBRE TOTAL"
                    })
            
            if resultados:
                st.error(f"⚠️ Se detectaron {len(resultados)} modelos del TOP 20 en Quiebre Total (Sin stock físico ni en tránsito).")
                df_final = pd.DataFrame(resultados).sort_values(by="Pares Vendidos (Histórico)", ascending=False).drop_duplicates()
                st.table(df_final)
            else:
                st.success("¡Excelente! Ningún modelo del Top 20 presenta quiebre total de surtido.")
    else:
