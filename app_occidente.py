...
# 2. ESTILO "PIEDRA Y LODO" (INAMOVIBLE)
st.markdown("""
    <style>
    /* Título con círculo rojo */
    .main-title {
        text-align: center; 
        color: #E30613; 
        font-size: 32px; 
    ...
    /* ENCABEZADOS: Fondo Rojo Institucional y Letra Blanca */
    th {
    ...
    /* ELIMINAR CUALQUIER ÍNDICE QUE DESPLACE LOS ENCABEZADOS */
    thead tr th:first-child { display: none !important; }
    tbody th { display: none !important; }
    
    /* Ajuste para evitar que el nombre se corte en columnas reducidas */
    table { width: 100% !important; }
    </style>
    <h1 class="main-title">🔴 MONITOR COMERCIAL OCCIDENTE</h1>
    """, unsafe_allow_html=True)

# 3. FUNCION PARA BUSCAR ARCHIVOS
...
with tab2:
    if archivo_modelos:
        ...
        tienda_sel = st.selectbox("Selecciona Tienda para Ranking:", sorted(df_agrupado[col_t].unique()))
        
        df_tienda = df_agrupado[df_agrupado[col_t] == tienda_sel].copy()
        
        # Limpieza de datos antes de mostrar
        top_20 = df_tienda[[col_mod, col_cant]].sort_values(by=col_cant, ascending=False).head(20).reset_index(drop=True)
        
        # RENOMBRAMIENTO FINAL DE COLUMNAS (MODELO Y PARES VENDIDOS)
        top_20.columns = ['MODELO', 'PARES VENDIDOS'] 

        # Estilo para pintar los 5 mejores (Toda la fila)
        ...
        st.subheader(f"🏆 TOP 20 PRODUCTOS - TIENDA {tienda_sel}")
        st.table(top_20.style.apply(resaltar_top_5, axis=None))

st.markdown("<p style='text-align: center; color: gray; font-size: 10px;'>Gestión Occidente | Lic. José Estrada</p>", unsafe_allow_html=True)
